...
public class BoundingBox {
	...
	public Vector2D getCenter() {
		...
		Vector2D center = new Vector2D((this.lower.x + (this.getWidth()*0.5)),
				(this.lower.y + (this.getHeight()*0.5)));
	...
	public static BoundingBox findBBox(Iterator<Block> iter) {
		...
		double smallestLowerX = Float.POSITIVE_INFINITY;
		double smallestLowerY = Float.POSITIVE_INFINITY;
		double largestUpperX = 0.0;
		double largestUpperY = 0.0;
		
		while (iter.hasNext()) {
			Block x = iter.next();
			if (x.getBBox().lower.x < smallestLowerX){
				smallestLowerX = x.getBBox().lower.x;
			}
			if (x.getBBox().upper.x > largestUpperX){
				largestUpperX = x.getBBox().upper.x;
			}
			if (x.getBBox().lower.y < smallestLowerY){
				smallestLowerY = x.getBBox().lower.y;
			}
		...
		Vector2D Low = new Vector2D(smallestLowerX,smallestLowerY);
