...
public class Shape {
	...
	private Vector2D d; // Displacement of the shape in the image coordinate
						// system.
	...
	/**
	 * Constructor: An instance containing blocks. Precondition: blocks is not
	 * empty.
	 */
	public Shape(HashSet<Block> blocks) {
		...
		// TODO: Construct the hierarchical tree structure.

		ArrayList<Block> BlocksArrayList = new ArrayList<Block>();
		for (Block b : blocks) {
			BlocksArrayList.add(b);
		}

		tree = new BlockTree(BlocksArrayList);
	...
	boolean overlapsOffscreen(double width, double height) {
		...
		return (box.lower.x < 0) || (box.upper.x > width) || (box.lower.y < 0)
				|| (box.upper.y > height);
	...
	/**
	 * Return true iff this shape overlaps shape t. This is a better
	 * implementation of overlap detection than naiveOverlaps.
	 */
	boolean betterOverlaps(Shape t) {
		// TODO: Implement me.

		if (tree.overlaps(this.d, t.tree, t.d)) {
			return true;
		}
	...
	private boolean betterContains(Vector2D p) {
		...
		if (this.tree.contains(p.minus(d))) {
			return true;
		}
