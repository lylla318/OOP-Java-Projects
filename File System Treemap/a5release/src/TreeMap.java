...
public class TreeMap {
    ...
    public static Wrapper2 getSplit(ArrayList<Node> b, int m, int n) {
    	...
    	for (int j = 0; j < b.size(); j++){
    		totalWeight += b.get(j).weight;
    	}
    	
    	int smallestK = 0;
    	double weightToK = 0.0;
    	double actualRatio = 0.0;
    	
    	if (totalWeight <= 0){
    		for (int k = 0; k <= (b.size()/2); k++){
        		weightToK += b.get(k).weight;
        	}
    		smallestK = (m+n)/2;
    		actualRatio = 0.5;
    	}
    		...
    		for (int k = 0; k <= b.size(); k++){
        		weightToK += b.get(k).weight;
        		if (weightToK >= (0.4 * totalWeight)){
        			smallestK = k;
        			actualRatio = weightToK/totalWeight;
        			break;
        		}
        	}
    	}
    	
        return new Wrapper2(smallestK, actualRatio);
    }
    ...
    private void sliceAndDice(ArrayList<Node> b, int m, int n,
    	...
    	//Base case. For b[m..n] of size 0, do nothing.
    	if (b.size() == 0){
    		return;
    	}
        ...
        //field of b[m] with color Color(0, 0, 127).
    	if (b.size() == 1){
    		Color c = new Color(0, 0, 127);
    		Block k = new Block(bbox,c);
    		b.get(m).block = k;
    		return;
    	}
    	
    	//Select the smallest k such that the total weight of b[m..k]
    	//is at least SPLIT_RATIO times the total weight of b[m..n].
    	//(Use function getSplit() to compute this.)
    	
    	int smallestK = getSplit(b,m,n).k;
    	
    	//Split the rectangle into two parts according to the actual
        //split-ratio (as returned by getSplit()) along its longer side
        //(to avoid very narrow shapes).
    	double ratio = getSplit(b,m,n).d;
    	
    	BoundingBox oldBox = new BoundingBox(bbox);
    	BoundingBox newBox = new BoundingBox(bbox);
    	
    	if (bbox.getLength() == bbox.getWidth()){
    		newBox.upper.x = bbox.lower.x + (bbox.getWidth()*ratio);
    		oldBox.lower.x += (bbox.getWidth() * ratio);
    	} 
    	
    	else if (bbox.getLength() == bbox.getHeight()){
    		newBox.upper.y = bbox.lower.y + (bbox.getHeight() * ratio);
    		oldBox.lower.y += (bbox.getHeight() * ratio);	
    	}
    	
    	//If k = n, subtract 1 from k to ensure progress toward
    	//termination in step f
    	...
    	//Recursively allocate nodes b[m..k] to the split-off part,
    	//and nodes b[k+1..n] to the rest of the rectangle.
        
    	ArrayList<Node> oldBoxList = new ArrayList<Node>();
    	
    	for (int i = 0; i <= smallestK; i++){
    		oldBoxList.add(b.get(i));
    	}
    	
    	sliceAndDice(oldBoxList, m, smallestK, oldBox, w, h);
    	
    	ArrayList<Node> newBoxList = new ArrayList<Node>();
    	
    	for (int i = 0; i <= smallestK; i++){
    		newBoxList.add(b.get(i));
    	}
    	
    	sliceAndDice(newBoxList, (smallestK+1), n, newBox, w, h);
    ...
    public static void main(String[] args) {
        ...
        File directory= new File("C:\\Users\\Lylla\\Desktop\\Test_Size");
    	System.out.println(FileTreeMap.size(directory,1));
