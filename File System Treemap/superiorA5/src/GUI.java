/** Time spent on a5: 3 h, 30 m. 
 *  Lylla Younes
 *  ley23
 *  This was a good assignment in that I learned about GUIs, but it was a little redundant. */
...
public class GUI extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	//find current treemap depth
    	int currentDepth = ftm.getMaxDepth();
    	
    	//depending on button clicked, increment or decrement depth
    	...
    	//recompute the treemap with the new depth
    	recomputeTreeMap(newDepth);
    ...
    public class MouseEvents extends MouseInputAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
        	...
        	Vector2D clicked = new Vector2D ((e.getX()/(treeView.getWidth()+0.0)),
        			(e.getY()/(treeView.getHeight() + 0.0)));
        	
        	//find the treemap node containing this point
        	TreeMap.Node n = ftm.getNodeContaining(clicked);
        	
        	//find the file corresponding to this node
        	File f = ftm.getFile(n);
        	
        	//select this file in the fileTree
        	FileTreePanel ftp = (FileTreePanel) fileTree;
        	ftp.select(f);
