...
public class TreeMap {
    ...
    public static Wrapper2 getSplit(ArrayList<Node> b, int m, int n) {
    	...
    	if (totalWeight <= 0.0){
    		...
    		for (int k = m; k <= n; k++){
        		weightToK += b.get(k).getWeight();
        		if (weightToK >= (SPLIT_RATIO * totalWeight)){
        			smallestK = k;
    ...
    private void sliceAndDice(ArrayList<Node> b, int m, int n,
    	...
    	double boxWidthScreen = bbox.getWidth()*w;
    	double boxHeightScreen = bbox.getHeight()*h;
    	
    	if (boxWidthScreen >= boxHeightScreen){
    	...
    	sliceAndDice(b, m, smallestK, newBox, w, h);
    	sliceAndDice(b, (smallestK + 1), n, oldBox, w, h);
