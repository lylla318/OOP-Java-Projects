 ...
 * Name(s):Lylla Younes
 * Netid(s): ley23	
 * What I thought about this assignment: Very good assignment, I really understand heaps now 
 * and I feel pretty comfortable with the implementation. 
 * 
 *
 */
...
public class MinHeap<E> implements PQueue<E> {
    ...
    public @Override void updatePriority(E e, double p) {
    	...
    	if ((eIndex-1)/2 <= 0){
    		//at least one child
    		if ((2*eIndex)+1 < size || (2*eIndex)+2 < size){
    			bubbleDown(eIndex);
    		}
    	}
        
    	//no children
    	if ((2*eIndex)+1 > size-1 || (2*eIndex)+2 > size-1){
    		bubbleUp(eIndex);
    	}
    	
    	//parent with children
    	if (map.get(b.get((eIndex-1)/2)).priority > p){
    		bubbleUp(eIndex);
    	}
    	else {
    		bubbleDown(eIndex);
    ...
    private void bubbleDown(int k) {
    	...
    	while (k < size){
    		...
    		if (leftC > (size-1) && rightC > (size-1)){ //no children
    			return;
    		} 
    		
    		else if (leftC < (size) && rightC < (size)){ //2 children
    			if (map.get(b.get(leftC)).priority < map.get(b.get(rightC)).priority){
    				smallerC = leftC;
    			} else {
    				smallerC = rightC;
    			}
    		...
    		if (K.priority > map.get(b.get(smallerC)).priority){
    			E temp = bk;
    			map.get(b.get(k)).index = smallerC;
    			map.get(b.get(smallerC)).index = k;
    			b.set(k, b.get(smallerC));
    			b.set(smallerC, temp);
    			k = smallerC;
